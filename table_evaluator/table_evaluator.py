...
class TableEvaluator:
    ...
    def plot_cumsums(self, nr_cols=4, model_dir_path="~"):
        """
        ...
        plt.savefig(model_dir_path + "/cumsum.jpg")
        plt.show()

    def plot_distributions(self, nr_cols=3, model_dir_path=""):
        """
        ...
        plt.tight_layout(rect=[0, 0.02, 1, 0.98])
        plt.savefig(model_dir_path + "/distribution.jpg")
        plt.show()

    def plot_correlation_difference(self, plot_diff=True, model_dir_path="~", **kwargs):
        """
        ...
        """
        plot_correlation_difference(self.real, self.fake, cat_cols=self.categorical_columns, plot_diff=plot_diff, model_dir_path=model_dir_path,
                                    **kwargs)
        plot_correlation_difference(self.real, self.fake, cat_cols=self.categorical_columns, plot_diff=True, model_dir_path=model_dir_path,
                                    **kwargs)
    ...
    def plot_pca(self, model_dir_path=""):
        """
        ...
        plt.savefig(model_dir_path + "/pca.jpg")
        plt.show()
    ...
    def score_estimators(self):
        ...
        if self.target_type == 'class':
            ...
            for r_classifier, f_classifier, estimator_name in zip(self.r_estimators, self.f_estimators,
                ...
                for dataset, target, dataset_name in zip([self.real_x_test],
                                                         [self.real_y_test], ['real']):
    ...
    def visual_evaluation(self, model_dir_path, **kwargs):
        """
        ...
        """
        self.plot_mean_std()
        plt.savefig(model_dir_path + "/plot_mean_std.jpg")
        self.plot_cumsums(model_dir_path=model_dir_path)
        self.plot_distributions(model_dir_path=model_dir_path)
        self.plot_correlation_difference(model_dir_path=model_dir_path, plot_diff=False, **kwargs)
        self.plot_pca(model_dir_path = model_dir_path)
