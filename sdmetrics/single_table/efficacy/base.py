...
class MLEfficacyMetric(SingleTableMetric):
    ...
    @classmethod
    def _fit_predict(cls, synthetic_data, synthetic_target, real_data, real_target, dtypes=None):
        ...
        if len(unique_labels) == 1:
            ...
            transformer = rdt.HyperTransformer(dtype_transformers={'O': 'one_hot_encoding'}, dtypes=dtypes)
    ...
    @classmethod
    def compute(cls, real_data, synthetic_data, metadata=None, target=None, scorer=None, dtypes=None):
        """Compute this metric.
        ...
        target_col_idx = real_data.columns.get_loc(target)

        dtypes.pop(target_col_idx)
        real_target = real_data.pop(target)
        synthetic_target = synthetic_data.pop(target)

        predictions = cls._fit_predict(synthetic_data, synthetic_target, real_data, real_target, dtypes)
        score = cls._score(scorer, real_target, predictions)
