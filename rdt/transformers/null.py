...
class NullTransformer(BaseTransformer):
    ...
    def fit(self, data):
        ...
        elif self.fill_value == "sample":
            self._fill_value = data.dropna().sample if pd.notnull(data).any() else 0
        else:
        ...
        if self.nulls:
            ...
            if self.nulls and self._fill_value is not None:
                if not self.copy:
                    ...
                    if self.fill_value == "sample":
                        data = pd.Series(np.where(data.isnull(), self._fill_value(len(data), replace=True), data))
                    else:
